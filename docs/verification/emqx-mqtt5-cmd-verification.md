...
- [x] Site-claim field: **resolved** -- certificate CN, formatted as the canonical `<site>/<node>` identifier (not SAN/OU as originally proposed; that mechanism is EE-only).
...
- **New, not previously tracked:** the wildcard-injection proof above should be treated as confirming, not just motivating, issue #25 -- recommend escalating its priority given a live exploit now exists in writing.
