...
description: Harden a NemoClaw sandbox after creation — deny-by-default network egress allow-listed to only the remote Nemotron inference host and the Teams webhook bridge, TLS in front of vLLM, and a hardening review mapped to IEC 62443 / NIS2 / CRA. Use when locking down an openAut agent's network access, reviewing sandbox security, setting up TLS for remote inference, or managing the four sandbox layers (filesystem, process, network, inference).
---
...
to the openAut defaults — **deny-by-default egress** that allows only the remote Nemotron host and
the Teams bridge — and stands up the **TLS** the inference link depends on.
...
control that turns "an agent with shell access" into "an agent that can only talk to its model and
its channel", and it is what satisfies the openAut NIS2 / IEC 62443 posture.
...
three destinations above. The acceptance test: from inside the sandbox, a request to any host other
than the allow-listed three must fail.
...
|---|---|---|
| Deny-by-default egress | `policy-list` shows only the 3 allow-listed destinations | NIS2 Art. 21, IEC 62443 SR 5.1 (zone/conduit) |
...
| Least-privilege channel | only the Teams bridge reachable, no other chat egress | IEC 62443 SR 7.x |
...
> inference link) is what to preserve.
